...
- **Tildes que desaparecen en los títulos.** SplitText envuelve cada palabra en una máscara con `overflow: clip` para la animación de subida; con el `line-height` apretado del título, el acento de una mayúscula queda fuera de la caja y se recorta. `styles.css` da aire arriba a `.title .word-mask` con un `padding-top` y un margen negativo que lo compensa. Si cambias la fuente o el interlineado y vuelven a faltar, sube ese valor. Además `choreo.ts` normaliza el texto a NFC por si llega con acentos combinantes. No renombres títulos para esquivarlo.
